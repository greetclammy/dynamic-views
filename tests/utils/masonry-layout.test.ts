import { calculateMasonryLayout, applyMasonryLayout } from '../../src/utils/masonry-layout';
...
describe('masonry-layout', () => {
  describe('calculateMasonryLayout', () => {
    ...
    it('should distribute cards evenly across columns', () => {
      ...
      const cards = Array(6).fill(null).map(() => createMockCard(100));
  ...
  describe('applyMasonryLayout', () => {
    it('should apply CSS custom properties to container', () => {
      ...
      expect(container.style.getPropertyValue('--masonry-height')).toBe(`${result.containerHeight}px`);
    });
    ...
    it('should apply CSS custom properties to cards', () => {
      ...
      expect(card1.style.getPropertyValue('--masonry-width')).toBe(`${result.cardWidth}px`);
