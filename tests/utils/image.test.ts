...
describe('image', () => {
  ...
  describe('hasValidImageExtension', () => {
    ...
    it('should work with full paths', () => {
      ...
      expect(hasValidImageExtension('https://example.com/image.jpg')).toBe(true);
    });
  ...
  describe('stripWikilinkSyntax', () => {
    ...
    it('should return unchanged path without wikilink syntax', () => {
      expect(stripWikilinkSyntax('image.png')).toBe('image.png');
      expect(stripWikilinkSyntax('path/to/image.png')).toBe('path/to/image.png');
    });
    ...
    it('should handle paths with folders', () => {
      expect(stripWikilinkSyntax('[[folder/image.png]]')).toBe('folder/image.png');
    });
  ...
  describe('processImagePaths', () => {
    ...
    it('should validate external URLs asynchronously', async () => {
      const paths = ['https://example.com/valid.png', 'https://example.com/invalid.png'];
    ...
    it.skip('should process external URLs without extensions', async () => {
      // Use real timers for async Image validation
      jest.useRealTimers();

      const paths = ['https://example.com/image'];
      const processPromise = processImagePaths(paths);

      // Trigger the Image onload callback asynchronously
      setTimeout(() => {
        const instances = (global as any).__imageInstances || [];
        instances.forEach((img: any) => {
          if (img && img.onload && img.src === 'https://example.com/image') {
            img.onload();
          }
        });
      }, 10);
      ...
      // URL should be included (validated successfully)
      expect(result.externalUrls).toEqual(['https://example.com/image']);
  ...
  describe('resolveInternalImagePaths', () => {
    ...
    it('should resolve internal image paths to resource URLs', () => {
      ...
      mockApp.vault.getResourcePath = jest.fn().mockReturnValue('app://local/image.png');

      const result = resolveInternalImagePaths(['image.png'], 'note.md', mockApp);

      expect(result).toEqual(['app://local/image.png']);
      expect(mockApp.metadataCache.getFirstLinkpathDest).toHaveBeenCalledWith('image.png', 'note.md');
    });
    ...
    it('should filter out non-image files', () => {
      ...
      const result = resolveInternalImagePaths(['document.pdf'], 'note.md', mockApp);
    ...
    it('should skip files that cannot be found', () => {
      ...
      const result = resolveInternalImagePaths(['missing.png'], 'note.md', mockApp);
    ...
    it('should handle multiple paths', () => {
      ...
      mockApp.vault.getResourcePath = jest.fn()
        .mockReturnValueOnce('app://local/image1.png')
        .mockReturnValueOnce('app://local/image2.jpg');

      const result = resolveInternalImagePaths(['image1.png', 'image2.jpg'], 'note.md', mockApp);

      expect(result).toEqual(['app://local/image1.png', 'app://local/image2.jpg']);
    });

    it('should accept all valid image extensions', () => {
      const extensions = ['avif', 'bmp', 'gif', 'jpeg', 'jpg', 'png', 'svg', 'webp'];

      extensions.forEach(ext => {
        const mockFile = { extension: ext } as TFile;
        mockApp.metadataCache.getFirstLinkpathDest = jest.fn().mockReturnValue(mockFile);
        mockApp.vault.getResourcePath = jest.fn().mockReturnValue(`app://local/image.${ext}`);

        const result = resolveInternalImagePaths([`image.${ext}`], 'note.md', mockApp);
  ...
  describe('extractEmbedImages', () => {
    ...
    it('should extract internal image embeds', async () => {
      ...
      mockApp.metadataCache.getFirstLinkpathDest = jest.fn().mockReturnValue(mockImageFile);
      mockApp.vault.getResourcePath = jest.fn().mockReturnValue('app://local/image.png');
    ...
    it('should filter out non-image embeds', async () => {
      ...
      mockApp.metadataCache.getFirstLinkpathDest = jest.fn().mockReturnValue(mockPdfFile);
  ...
  describe('loadImageForFile', () => {
    ...
    it('should prioritize property images over embeds', async () => {
      ...
      const result = await loadImageForFile(mockApp, 'note.md', 'property.png', 'balanced', true, 'cover');
    ...
    it('should fall back to embeds when property is empty', async () => {
      ...
      mockApp.metadataCache.getFirstLinkpathDest = jest.fn().mockReturnValue(mockEmbedImage);
      mockApp.vault.getResourcePath = jest.fn().mockReturnValue('app://local/embed.jpg');

      const result = await loadImageForFile(mockApp, 'note.md', '', 'balanced', true, 'cover');
    ...
    it('should not fall back when fallbackToEmbeds is false', async () => {
      ...
      const result = await loadImageForFile(mockApp, 'note.md', '', 'balanced', false, 'cover');
    ...
    it('should return array when multiple images available', async () => {
      ...
      const result = await loadImageForFile(mockApp, 'note.md', ['image1.png', 'image2.jpg'], 'balanced', true, 'cover');
    ...
    it('should use embeds when no image property configured', async () => {
      ...
      mockApp.metadataCache.getFirstLinkpathDest = jest.fn().mockReturnValue(mockEmbedImage);
      mockApp.vault.getResourcePath = jest.fn().mockReturnValue('app://local/embed.png');

      const result = await loadImageForFile(mockApp, 'note.md', '', 'balanced', true, '');
