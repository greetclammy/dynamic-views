...
describe('storage', () => {
  describe('getStorageKey', () => {
    ...
    it('should handle different ctimes', () => {
      expect(getStorageKey(1000000000, 'test')).toBe('dynamic-views-1000000000-test');
      expect(getStorageKey(999999999999, 'test')).toBe('dynamic-views-999999999999-test');
      expect(getStorageKey(0, 'test')).toBe('dynamic-views-0-test');
    });

    it('should handle different keys', () => {
      const ctime = 1234567890;
      expect(getStorageKey(ctime, 'viewMode')).toBe('dynamic-views-1234567890-viewMode');
      expect(getStorageKey(ctime, 'filterState')).toBe('dynamic-views-1234567890-filterState');
      expect(getStorageKey(ctime, 'customKey')).toBe('dynamic-views-1234567890-customKey');
    });
  ...
  describe('getGlobalStorageKey', () => {
    ...
    it('should handle different global keys', () => {
      ...
      expect(getGlobalStorageKey('preferences')).toBe('dynamic-views-global-preferences');
